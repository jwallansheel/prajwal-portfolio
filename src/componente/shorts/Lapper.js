...
const carColors = [
  "#FF1242", "#00D2BE", "#0072C6", "#F596C8", "#E10600",
  "#00B0F0", "#B6BABD", "#0600EF", "#9B0000", "#006F62",
  "#0090D2", "#005AFF", "#DC0000", "#44C8F5", "#FFFFFF",
  "#3671C6", "#C92D4B", "#006F62", "#2293D1", "#005AFF"
];

// Initial speeds for each car
const initialSpeeds = [1, 1.2, 1.4, 1.6, 1.8, 2, 1, 1.5, 1.2, 1.4, 1.6, 1.8, 2, 1.5, 1, 1.2, 1.4, 1.6, 1.8, 2];
...
const App = () => {
      ...
      setCarPositions((prevPositions) => {
        return prevPositions.map((position, index) => {
          ...
          if (newPosition >= trackPath.length) {
            const lapTime = ((Date.now() - lapStartTimes.current[index]) / 1000).toFixed(2);
  ...
  const Car = ({ color, position }) => {
    ...
    return <circle cx={currentPosition.x} cy={currentPosition.y} r="10" fill={color} />;
  };

  const LapTimeDisplay = ({ color, lapTimes }) => (
    <div className="lap-time-container">
      <h2 style={{ color }}>{color.charAt(0).toUpperCase() + color.slice(1)} Car Lap Times</h2>
      <ul>
        {lapTimes.slice(-5).map((time, lapIndex) => ( // Show only the last three lap times
  ...
  const SpeedSlider = ({ index, speed, onSpeedChange }) => (
    <div className="speed-slider">
      <label htmlFor={`speed-${index}`} style={{ color: carColors[index] }}>
        {carColors[index].charAt(0).toUpperCase() + carColors[index].slice(1)} Car Speed
      </label>
      <input
        type="range"
        id={`speed-${index}`}
  ...
  const InformationPanel = () => (
    <div className="info-panel">
      ...
      <p>This is a simulation of the Spa-Francorchamps racing track.</p>
      <p>Each car has a different speed that can be adjusted using the sliders.</p>
      <p>Lap times are recorded and displayed for each car.</p>
      <p>Use the simulation to see how different speeds affect lap times!</p>
    </div>
  );
    ...
    <div className="App">
      ...
      <div className="main-content">
        ...
        <div className="track-section">
          <svg width="600" height="500" className="race-track">
            <path
              d={`M ${trackPath.map(point => `${point.x} ${point.y}`).join(' L ')}`}
              fill="none"
              stroke="black"
            />
            {carColors.map((color, index) => (
      ...
      <div className="lap-times-section">
        <div className="lap-times-container">
          {carColors.map((color, index) => (
            <LapTimeDisplay key={color} color={color} lapTimes={lapTimes[index]} />
          ))}
        </div>
      </div>
    </div>
